...
SCRIPT_PATH = ROOT / "scripts" / "run_p3_beta_mvp_query_review_evidence_ui_polish_followup.py"
JSON_PATH = ROOT / "data" / "admin" / "query_review_evidence_ui_polish_followup_v0.json"
...
def load_module():
    spec = importlib.util.spec_from_file_location(
        "query_review_evidence_ui_polish_followup",
        SCRIPT_PATH,
    )
...
class QueryReviewEvidenceUiPolishFollowupTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_default_decision_status_ready_for_owner_approved_push(self) -> None:
        self.assertEqual(
            ...
            "query_review_evidence_ui_polish_followup_passed_ready_for_owner_approved_push",
        )

    def test_templates_include_copy_summary_button(self) -> None:
    ...
    def test_35_lux_aa_summary_is_human_readable(self) -> None:
        row = self.rows["35 lux aa"]
        review = row["display_query_review"]
        self.assertIn("Leica Summilux 35 AA candidate", review["interpreted_target"])
        self.assertIn("AA-specific price evidence is not enough yet.", review["why"])
        self.assertIn("Price evidence found:", review["evidence_summary"])
        self.assertIn("Price stays locked until at least 2 exact variant listings have reliable prices.", review["copy_summary_text"])

    def test_noctilux_e60_and_rigid_use_user_facing_labels(self) -> None:
        e60 = self.rows["Noctilux 50 f1 E60"]
        rigid = self.rows["Summicron 50 rigid"]
        self.assertNotIn("990,000", e60["display_broader_reference_band"] or "")
    ...
    def test_m50_12_is_lens_and_reference_only(self) -> None:
        ...
        self.assertEqual(row["display_broader_reference_label"], "Broader family reference")
        self.assertFalse(row["display_price_summary_allowed"])
    ...
    def test_no_internal_tokens_in_user_facing_summary(self) -> None:
        for row in self.payload["query_results"]:
            text = " | ".join(
                [
                    (row.get("display_query_review") or {}).get("copy_summary_text") or "",
    ...
    def test_script_generates_pass_payload(self) -> None:
        ...
        self.assertEqual(payload["price_projection_regressed_rows"], [])
        self.assertEqual(payload["body_lens_regression_rows"], [])
    ...
    def test_push_context_can_promote_status(self) -> None:
        ...
        self.assertEqual(
            ...
            "query_review_evidence_ui_polish_followup_pushed_ready_for_owner_recheck",
        )


if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(QueryReviewEvidenceUiPolishFollowupTests)
